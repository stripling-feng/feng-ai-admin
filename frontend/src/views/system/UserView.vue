  ...
  <div class="page-card">
      ...
      <div class="filter-card">
        <el-form :inline="true" :model="filters" class="filter-form" label-position="top">
          <el-form-item label="用户名" class="field-username">
            <el-input v-model="filters.username" clearable placeholder="用户名模糊搜索" @keyup.enter="handleSearch" />
          </el-form-item>
          ...
          <el-form-item label="岗位" class="field-post">
            <el-select v-model="filters.postId" clearable placeholder="岗位筛选">
              <el-option v-for="post in posts" :key="post.id" :label="post.postName" :value="post.id" />
            </el-select>
          </el-form-item>
          <el-form-item label="角色" class="field-role">
            <el-select v-model="filters.roleId" clearable placeholder="角色筛选">
    ...
    <el-table :data="list" border @selection-change="handleSelectionChange">
      ...
      <el-table-column label="角色" min-width="180">
        <template #default="{ row }">{{ getRoleNames(row.roleIds).join('、') || '-' }}</template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="240" fixed="right">
        <template #default="{ row }">
          ...
          <el-button v-permission="'system:user:edit'" link type="warning" @click="handleResetPassword(row.id)">
            重置密码
          </el-button>
          <el-button v-permission="'system:user:remove'" link type="danger" @click="handleDelete(row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <CrudDialog v-model="visible" :title="form.id ? '编辑用户' : '新增用户'" @submit="handleSubmit">
      <el-alert
        v-if="!form.id"
    ...
    <el-dialog v-model="resetVisible" :title="`重置密码${resetTarget === 'batch' ? '（批量）' : ''}`" width="460px" append-to-body @close="resetResetForm">
      <el-form ref="resetFormRef" :model="resetPwdForm" :rules="resetFormRules" label-width="100px">
        ...
        <el-form-item v-if="resetPwdForm.mode === 'custom'" label="自定义密码" prop="password">
          <el-input v-model="resetPwdForm.password" type="password" show-password placeholder="请输入自定义密码" />
        </el-form-item>
      </el-form>
      <template #footer>
...
import { deptApi, postApi, roleApi, systemConfigApi, userApi } from '../../api/system'
...
function getRoleNames(roleIds = []) {
  return roles.value.filter((item) => roleIds.includes(item.id)).map((item) => item.roleName)
}
...
async function confirmResetPassword() {
  ...
  const password = resetPwdForm.mode === 'custom' ? resetPwdForm.password : null
  const showPassword = resetPwdForm.mode === 'custom' ? resetPwdForm.password : defaultPassword.value
